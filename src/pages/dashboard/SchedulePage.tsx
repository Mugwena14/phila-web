...
const STATUS_MAP: Record<string, { label: string; color: string }> = {
  ...
  no_show:         { label: 'No-show',         color: colors.coral },
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '24px 32px 0', flexShrink: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20 }}>
            ...
            <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
              <button onClick={() => changeDate(-1)} style={{ width: 34, height: 34, borderRadius: 8, border: `1px solid ${colors.border}`, background: 'none', cursor: 'pointer', color: colors.textMuted, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <button onClick={() => setSelectedDate(new Date().toISOString().split('T')[0])} style={{ height: 34, padding: '0 14px', borderRadius: 8, border: `1px solid ${colors.border}`, background: 'none', cursor: 'pointer', color: colors.textMuted, fontSize: 12, display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '0 32px 32px' }}>
          <div style={{ backgroundColor: colors.bgSurface, borderRadius: 12, border: `1px solid ${colors.border}`, overflow: 'hidden' }}>
            ...
            {/* Table header */}
            <div style={{ display: 'grid', gridTemplateColumns: '80px 1fr 120px 110px 130px', backgroundColor: '#0A1A17', padding: '12px 20px', gap: 16 }}>
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                      {slot.status === 'available' && (
                        <span style={{ fontSize: 13, color: colors.textFaint }}>Open slot</span>
                      )}
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                          ...
                          <div style={{ minWidth: 0 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                              ...
                              {booking.crisis_flag && <Ico i={HiOutlineExclamation} size={13} style={{ color: colors.coral }} />}
                              {booking.is_walk_in && (
                                <span style={{ fontSize: 9, fontWeight: 700, color: '#60A5FA', backgroundColor: 'rgba(59,130,246,0.1)', borderRadius: 999, padding: '1px 6px', border: '1px solid rgba(59,130,246,0.2)' }}>WALK-IN</span>
                              )}
                            </div>
                            {booking.reason && (
                              <span style={{ fontSize: 11, color: colors.textFaint, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', display: 'block', maxWidth: 220 }}>{booking.reason}</span>
                            )}
                          </div>
                        </div>
                      )}
                    ...
                    <div>
                      {slot.status === 'booked' && booking && st && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          <div style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: st.color, flexShrink: 0 }} />
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      {slot.status === 'available' && (
                        <>
                          <button
                            onClick={() => { setSelectedSlot(slot); setShowWalkInModal(true) }}
                            title="Add walk-in"
                            style={{ width: 32, height: 32, borderRadius: 8, border: `1px solid ${colors.border}`, background: 'none', cursor: 'pointer', color: colors.textMuted, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                          >
                            <Ico i={HiOutlinePlus} size={15} />
                          </button>
                          <button
                            onClick={() => { setSelectedSlot(slot); setShowBlockModal(true) }}
                            title="Block slot"
                            style={{ width: 32, height: 32, borderRadius: 8, border: `1px solid ${colors.border}`, background: 'none', cursor: 'pointer', color: colors.textMuted, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                          >
            ...
            <div style={{ marginBottom: 20, paddingBottom: 20, borderBottom: `1px solid ${colors.border}` }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                ...
                {selectedBooking.is_walk_in && (
                  <span style={{ fontSize: 11, fontWeight: 600, color: '#60A5FA', backgroundColor: 'rgba(59,130,246,0.1)', borderRadius: 999, padding: '3px 10px', border: '1px solid rgba(59,130,246,0.2)' }}>Walk-in</span>
                )}
              </div>
            </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {STATUS_ACTIONS.filter(a => a.status !== selectedBooking.status).map(action => (
                  <button
                    ...
                    style={{ padding: '9px 14px', borderRadius: 8, border: `1px solid ${colors.border}`, backgroundColor: 'transparent', color: action.color, fontSize: 12, fontWeight: 500, cursor: 'pointer', textAlign: 'left', display: 'flex', alignItems: 'center', gap: 8, opacity: statusLoading === action.status ? 0.6 : 1 }}
                  >
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                {selectedBooking.intake_brief.crisis_flagged && (
                  <div style={{ backgroundColor: colors.coralBg, border: `1px solid ${colors.coralBorder}`, borderRadius: 8, padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 8 }}>
                    <Ico i={HiOutlineExclamation} size={15} style={{ color: colors.coral }} />
                    <span style={{ fontSize: 12, color: colors.coral, fontWeight: 500 }}>Crisis flag detected during intake</span>
                  </div>
                )}
              </div>
            ) : (
          ...
          <div style={{ backgroundColor: colors.bgSurface, borderRadius: 16, border: `1px solid ${colors.border}`, padding: 28, width: 420, maxWidth: '90vw' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <div>
                ...
                <div style={{ fontSize: 12, color: colors.textMuted, marginTop: 2 }}>
                  {selectedSlot ? `${selectedSlot.start_time.slice(0, 5)} · ${selectedSlot.date}` : ''}
                </div>
              </div>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
              ...
              >
                ...
                {walkInLoading ? 'Creating...' : 'Confirm walk-in'}
              </button>
            </div>
          </div>
        </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
              <div>
                <div style={{ fontSize: 16, fontWeight: 700, fontFamily: 'Syne, sans-serif', color: colors.text }}>
                  {selectedSlot.status === 'blocked' ? 'Unblock slot' : 'Block slot'}
                </div>
                <div style={{ fontSize: 12, color: colors.textMuted, marginTop: 2 }}>
                  {selectedSlot.start_time.slice(0, 5)} · {selectedSlot.date}
                </div>
              </div>
            ...
            {selectedSlot.status === 'blocked' ? (
              <>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  <button onClick={() => setShowBlockModal(false)} style={{ flex: 1, padding: '11px', borderRadius: 8, border: `1px solid ${colors.border}`, backgroundColor: 'transparent', color: colors.textMuted, fontSize: 13, cursor: 'pointer' }}>Cancel</button>
